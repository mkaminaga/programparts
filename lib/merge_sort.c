...
/* void merge(data_t[], uint32_t, uint32_t)
...
void merge(data_t a[], uint32_t start, uint32_t len);

/* void msort(data_t[], uint32_t, uint32_t)
 * divide and merge recrusivly.
 * arg1: pointer to the array
 * arg2: index of segment head
 * arg3: length of segmant */
void msort(data_t a[], uint32_t start, uint32_t length);

/* void merge_sort(data_t[], uint32_t)
 * sort elements of array using merge sort.
 * arg1: pointer of the array
 * arg2: length of array */
void merge_sort(data_t a[], uint32_t length) {
    msort(a, 0, length);
}
